...
public class Core extends Thread {	
		...
		for(;;) {
			try {
				...
				if(!indObj.chkIndex) {
					...
					lblNewLabel_1.setText("Next indexing in "+indObj.sleepMin+" minutes.");
				}
	...
	public Core() {
		...
		frame.setBounds(100, 100, 1241, 760);
		...
		chckbxContains.setSelected(true);
		...
		chckbxAny = new JCheckBox("With Any Extension");
		...
		chckbxAny.setBounds(609, 58, 174, 25);
		frame.getContentPane().add(chckbxAny);		
		chckbxByExtension = new JCheckBox("Search By Extension");
		...
		chckbxByExtension.setBounds(808, -2, 165, 25);
		...
		btnIndexEvery.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				sleepMin=(Integer.parseInt(textField_1.getText()));
				indObj.sleepMinSetter((int)sleepMin);				
				lblNewLabel_1.setText("Next indexing in "+sleepMin+" minutes.");
			}
		});
		...
		chckbxGetDuplicates.setBounds(808, 28, 121, 25);
		frame.getContentPane().add(chckbxGetDuplicates);	
		frame.setFocusTraversalPolicy(new FocusTraversalOnArray(new Component[]{frame.getContentPane(), textField_1, textField, btnSrch, lblNewLabel, chckbxWhole, chckbxContains, chckbxAny, chckbxByExtension, btnIndexEvery, lblMinutes, lblNewLabel_1, chckbxGetDuplicates}));
	}
